...
class SplashScreen(ctk.CTk):
    def __init__(self):
        ...
        self.logo = ctk.CTkLabel(self, text="WPT", font=("Segoe UI", 44, "bold"))
        ...
        self._pulse(0.0)
        self.after(1400, lambda: self._fade_out(1.0))
    ...
    def _pulse(self, value):
        ...
        self.after(40, lambda: self._pulse(next_val))
    ...
    def _fade_out(self, alpha):
        ...
        try:
            ...
            self.destroy()
            return
        self.after(40, lambda: self._fade_out(alpha))
...
class WebPentestTool(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        sidebar = ctk.CTkFrame(self, width=300, corner_radius=0, fg_color="#0b0b0b")
        sidebar.grid(row=0, column=0, sticky="nswe")
        sidebar.grid_rowconfigure(8, weight=1)

        title = ctk.CTkLabel(sidebar, text="ShadowScan", font=("Segoe UI", 18, "bold"))
        title.grid(row=0, column=0, padx=20, pady=(20, 8), sticky="w")

        ctk.CTkLabel(sidebar, text="Target URL", font=("Segoe UI", 12, "bold")).grid(row=1, column=0, padx=20, pady=(8, 6), sticky="w")
        self.url_entry = ctk.CTkEntry(sidebar, placeholder_text="https://example.com", fg_color="#141414")
        self.url_entry.grid(row=2, column=0, padx=20, pady=(0, 12), sticky="we")
        self.url_entry.insert(0, "http://127.0.0.1:5001")

        ctk.CTkLabel(sidebar, text="Threads", font=("Segoe UI", 12, "bold")).grid(row=3, column=0, padx=20, pady=(8, 6), sticky="w")
        self.thread_slider = ctk.CTkSlider(sidebar, from_=1, to=30, number_of_steps=29, fg_color="#2a2a2a", progress_color="#b30000")
        ...
        self.start_btn = ctk.CTkButton(button_row, text="Run Audit", command=self.start_scan, fg_color="#b30000", hover_color="#e60000")
        self.start_btn.grid(row=0, column=0, padx=(0, 6), pady=6, sticky="we")
        self.stop_btn = ctk.CTkButton(button_row, text="Stop", fg_color="#3b3b3b", hover_color="#4a4a4a", command=self.stop_scan)
        ...
        ctk.CTkButton(control_row, text="Select All", fg_color="#2a2a2a", hover_color="#3a3a3a", command=self.select_all).grid(row=0, column=0, padx=(0, 6), pady=4, sticky="we")
        ctk.CTkButton(control_row, text="Clear All", fg_color="#2a2a2a", hover_color="#3a3a3a", command=self.clear_all).grid(row=0, column=1, padx=(6, 0), pady=4, sticky="we")
        ...
        self.export_btn = ctk.CTkButton(export_row, text="Export JSON", fg_color="#2a2a2a", hover_color="#3a3a3a", command=self.export_json)
        self.export_btn.grid(row=0, column=0, pady=4, sticky="we")

        note = ctk.CTkLabel(sidebar, text="Authorized testing only", font=("Segoe UI", 10))
        note.grid(row=9, column=0, padx=20, pady=(0, 14), sticky="w")

        main = ctk.CTkFrame(self, corner_radius=0, fg_color="#0f0f0f")
        ...
        ctk.CTkLabel(header, text="Live Audit", font=("Segoe UI", 20, "bold")).grid(row=0, column=0, sticky="w")
        self.status_label = ctk.CTkLabel(header, text="Idle", font=("Segoe UI", 12))
        self.status_label.grid(row=0, column=1, sticky="e")

        self.progress = ctk.CTkProgressBar(main, progress_color="#b30000", fg_color="#2a2a2a")
        self.progress.grid(row=1, column=0, sticky="we", padx=20)
        self.progress.set(0)

        content = ctk.CTkFrame(main, fg_color="#0f0f0f")
        content.grid(row=2, column=0, sticky="nswe", padx=20, pady=(10, 20))
        content.grid_columnconfigure(1, weight=1)
        content.grid_rowconfigure(0, weight=1)

        modules_panel = ctk.CTkFrame(content, width=380, fg_color="#111111")
        modules_panel.grid(row=0, column=0, sticky="nswe", padx=(0, 12))
        modules_panel.grid_rowconfigure(2, weight=1)

        ctk.CTkLabel(modules_panel, text="Modules", font=("Segoe UI", 16, "bold")).grid(row=0, column=0, padx=16, pady=(16, 8), sticky="w")
        self.search_entry = ctk.CTkEntry(modules_panel, placeholder_text="Search modules...", fg_color="#141414")
        self.search_entry.grid(row=1, column=0, padx=16, pady=(0, 8), sticky="we")
        self.search_entry.bind("<KeyRelease>", self.filter_modules)

        self.modules_scroll = ctk.CTkScrollableFrame(modules_panel, corner_radius=0, fg_color="#111111")
        self.modules_scroll.grid(row=2, column=0, sticky="nswe", padx=10, pady=(0, 10))
        self._render_modules()

        results_panel = ctk.CTkFrame(content, fg_color="#111111")
        ...
        ctk.CTkLabel(results_panel, text="Results", font=("Segoe UI", 16, "bold")).grid(row=0, column=0, padx=16, pady=(16, 8), sticky="w")
        self.results_text = ctk.CTkTextbox(results_panel, wrap="word", fg_color="#0d0d0d")
    ...
    def _render_modules(self):
        ...
        for category, items in MODULE_CATALOG:
            header = ctk.CTkLabel(self.modules_scroll, text=category, font=("Segoe UI", 13, "bold"))
            ...
            for key, label in items:
                ...
                cb = ctk.CTkCheckBox(self.modules_scroll, text=label, variable=var, fg_color="#b30000", hover_color="#e60000")
    ...
    def start_scan(self):
        ...
        self.results_text.insert("end", f"[*] AUDIT INITIALIZED: {url}\n{'='*70}\n")
        self.last_results = None
...
<root>&xxe;</root>"""
    ...
    def _scan_cors(self):
        ...
        elif aca_origin and aca_origin.strip() == origin:
            findings.append({"type": "CORS Misconfig (Origin Reflected)", "location": self.base_url})
        return findings
    ...
    def _scan_generic(self, key):
        ...
        if key == "server_banner" and headers.get("Server"):
            ...
            for tag in soup.find_all(["script", "link", "img"]):
                ...
                if ref and ref.startswith("http") and not ref.startswith(self.base_url):
            ...
            if res2 and "injected" in str(res2.headers):
                findings.append({"type": "Possible Header Injection", "location": test_url})
        return findings
